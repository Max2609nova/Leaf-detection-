...
- **Fixed a stale-timestamp bug** — every analysis used to report the same `analysis_timestamp` (frozen at server startup) because of how the dataclass default was defined; each result now gets its own real timestamp.
- **Fixed a bug in `utils.py`** where the CLI test path was base64-encoding a *file path string* instead of the actual image bytes.
- **Image validation & optimization** — uploads are now checked for size/type, and oversized images are automatically downscaled (Pillow) before being sent to the model, which speeds up requests and reduces cost.
- **Backend hardening (`app.py`)** — request size/content-type validation, proper 400 vs 500 error codes, CORS support, a `/health` endpoint, and a new `/disease-detection-base64` JSON endpoint.
- **Frontend overhaul (`main.py`)** — fixed duplicate imports and an `http://` URL that should have been `https://`; added a configurable API URL, request timeouts with friendly error messages, a "try a sample image" button, a confidence meter, color-coded severity badges, a downloadable JSON report, and a session history sidebar.
- **Config wiring** — model name/temperature/token limits are now actually read from environment variables instead of being silently ignored.
